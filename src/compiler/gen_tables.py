...
w(";; That is all a per-BYTE table can say.  The lexer decodes the character")
w(";; and asks uflags_of for XID_Start / XID_Continue, so the answer here is a")
w(";; fast path and not the rule -- it used to BE the rule, and an invisible")
w(";; NBSP was a variable name.")
w(";; ---------------------------------------------------------------------------")
w("align 64")
w("global cc_table")
w("cc_table:")
